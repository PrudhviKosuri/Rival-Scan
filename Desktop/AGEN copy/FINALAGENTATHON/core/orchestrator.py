...
async def run_analysis_job(job_id: str, entity: str):
    """
    Background task to run orchestration and update job status
    """
    try:
        ...
        # Route logic (mimic orchestrate simple flow)
        all_types = [AgentType.COMPANY_OVERVIEW, AgentType.REVENUE_TURNOVER, 
                     AgentType.PRICING_CHANGE, AgentType.PRODUCT_LAUNCH]
        message = f"Analyze {entity}"
        if context and context.get("cached_facts"):
        ...
        # Structure the final result similar to /orchestrate response
        final_result = {
        ...
        for agent_type_str, result in router_results.get("agent_results", {}).items():
            # Minimal enrichment for storage
            final_result["results"][agent_type_str] = {
                "agent_name": agent_type_str,
                "agent_output": result,
                "timestamp": datetime.now().isoformat()
            }
        ...
        try:
            derived_es = build_executive_summary(job_id, final_result["results"])
            results_fin = (final_result["results"].get("revenue_turnover", {}) or {}).get("agent_output", {}) or {}
            results_pr = (final_result["results"].get("pricing_change", {}) or {}).get("agent_output", {}) or {}
            results_sent = (final_result["results"].get("sentiment", {}) or {}).get("agent_output", {}) or {}
            derived_risks = classify_risk_severity(results_sent.get("risks") or [], results_fin, results_pr)
            derived_followups = generate_followups(final_result["results"])
            final_result["results"]["executive_summary"] = {
                "agent_name": "executive_summary",
                "agent_output": derived_es,
                "timestamp": datetime.now().isoformat()
            }
            final_result["results"]["risk_severity"] = {
                "agent_name": "risk_severity_classifier",
                "agent_output": {"risks": derived_risks},
                "timestamp": datetime.now().isoformat()
            }
            final_result["results"]["follow_ups"] = {
                "agent_name": "follow_up_questions",
                "agent_output": {"questions": derived_followups},
                "timestamp": datetime.now().isoformat()
            }
            conf_bundle = {}
            conf_bundle["overview"] = compute_confidence((final_result["results"].get("company_overview", {}) or {}).get("agent_output", {}) or {})
            conf_bundle["offerings"] = compute_confidence({
                "pricing_changes": (final_result["results"].get("pricing_change", {}) or {}).get("agent_output", {}) or {},
                "product_launches": (final_result["results"].get("product_launch", {}) or {}).get("agent_output", {}) or {}
            })
            conf_bundle["market_signals"] = compute_confidence((final_result["results"].get("revenue_turnover", {}) or {}).get("agent_output", {}) or {})
            conf_bundle["sentiment"] = compute_confidence(results_sent or {})
            final_result["results"]["confidence_scores"] = {
                "agent_name": "confidence_estimator",
