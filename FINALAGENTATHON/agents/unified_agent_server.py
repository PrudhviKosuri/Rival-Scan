...
# --- CONFIG ---
GOOGLE_API_KEY = os.environ.get("GOOGLE_API_KEY", "AIzaSyAtXNICmFx2i7Gi1sdyb3nYZFAxHr8I1M4")
MODEL_CANDIDATES = [
        ...
        for model_name in MODEL_CANDIDATES:
            try:
                resp = client.responses.generate(
                    model=model_name,
                    system_instruction="Return ONLY valid JSON matching schema. No markdown.",
                    input=prompt,
                ...
                last_err = e_inner
                continue
        return {"error": str(last_err) if last_err else "generation failed"}
